...
# Define function to plot grid approximation 
grid_approx <- function(correct, total, teacher_name) {
  # specifications (same as in Q.1a)
  dens <- 20
  ...
  # plot
  plot(p_grid, posterior,
       main = paste("Teacher: ", teacher_name),
       type = "b",
       xlab = "Probability of cogsci knowledge",
       ylab = "Posterior probability")
}
...
grid_approx <- function(correct, total, teacher_name) {
  # specifications, almost the same as previous question
  dens <- 100
  p_grid <- seq(from = 0, to = 1, length.out = dens)
  prior <- dnorm(p_grid, 0.8, 0.2) # this is the change - normal distribution with mean of 0.8 and 0.2 sd
  likelihood <- dbinom(correct, size = total, prob = p_grid)
  unstd.posterior <- likelihood * prior
  posterior <- unstd.posterior / sum(unstd.posterior)
  # plot
...
The results change, especially for Riccardo and Kristian because they have so few datapoints. This has the effect that the priors make a larger difference.
...
grid_approx <- function(correct, total, teacher_name) {
  # specifications
  correct <- correct * 100 # multiply by a 100
  total <- total * 100
  dens <- 100  # change density to get a bit better estimates
  p_grid <- seq(from = 0, to = 1, length.out = dens)
  
  # uniform prior
  prior <- rep(1, dens)  # uniform prior
  likelihood <- dbinom(correct, size = total, prob = p_grid)
  unstd.posterior <- likelihood * prior
  posterior <- unstd.posterior / sum(unstd.posterior)
  # plot
  ...
  # normal prior
  prior <- dnorm(p_grid, 0.8, 0.2)  # normal prior with a mean of 0.8 and sd 0f 0.2
  likelihood <- dbinom(correct, size = total, prob = p_grid)
  unstd.posterior <- likelihood * prior
  posterior <- unstd.posterior / sum(unstd.posterior)
  # plot
...
We now have enough datapoints to be fairly sure about the teachers true knowledge of cogsci so the priors make less of a difference now.
